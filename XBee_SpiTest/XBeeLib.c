...
//	PORTB = SPI_SS | XBEE_RESET | XBEE_ATTN;
		...
		while ( 1 )
		{
			...
			if ( bRet == true )
			{
				...
				if ( iRet == 0)
				{
					...
					break;
					//// Api Frame識別子は同じ？
					//if ( ptXBeeResApiFrame->pszData[ 0 ] == ptXBeeApiFrame->pszData[ 0 ] )
					//{
						//bRet = true;
						//break;
					//}
					//free( ptXBeeResApiFrame->pszData );
					//memset( ptXBeeResApiFrame, 0x00, sizeof(PXBEE_API_FRAME_TABLE) );
				}
			}
